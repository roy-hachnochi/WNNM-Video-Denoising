...
sConfig.sNoise.sigma =   30;   	% Gaussian noise STD (in [0,255] scale)
...
sConfig.sBlockMatching.patchSize =         7;    % Patch size
sConfig.sBlockMatching.maxNeighborsFrame = 170;   % Maximal number of nearest neighbors per frame
sConfig.sBlockMatching.maxGroupSize =      170;  % Maximal group size (number of patches) per reference patch
...
sConfig.sWNNM.maxUngrouped =   0.2;       % Maximal allowed % of ungrouped pixels per frame to end algorithm
sConfig.sWNNM.delta =          0.1;       % Iterative regularization parameter
sConfig.sWNNM.C =              2*sqrt(2); % Weight constant
sConfig.sWNNM.BMIter =         1;         % Number of iterations between re-block-matching
sConfig.sWNNM.lambda =         0.56;      % Noise estimate parameter
